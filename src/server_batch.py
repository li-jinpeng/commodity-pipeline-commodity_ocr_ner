...
class ModelServing(model_serving_pb2_grpc.ModelServingServicer):
            ...
            for word in word_res:
                ...
                for prefix in prefix_list:
                    if prefix in word:
                        ...
                        if len(text) == 0:
                            continue
                        if '生产厂商' not in ner_dict:
                            ner_dict['生产厂商'] = set()
                        ner_dict['生产厂商'].add(text)
                        break
                ...
                for prefix in prefix_list:
                    if prefix in word:
                        ...
                        for i in range(idx, len(word)):
                            if word[i] not in ['升', 'l', 'L', '毫升', 'ml', "mL", '克', 'g', '千克', 'kg']:
                                continue
                            break
                        text = word[word.find(prefix) + len(prefix) : i + 1]
                        if len(text) == 0:
                            continue
                        if '净含量' not in ner_dict:
                            ner_dict['净含量'] = set()
                        ner_dict['净含量'].add(text)
                        break
                ...
                for prefix in prefix_list:
                    if prefix in word:
                        ...
                        if idx == len(word):
                            continue
                        for i in range(idx, len(word)):
                            if word[i] not in ['年', '月', '日', '天', '小时']:
                                continue
                            break
                        text = word[word.find(prefix) + len(prefix) : i + 1]
                        if len(text) == 0:
                            continue
                        if '保质期' not in ner_dict:
                            ner_dict['保质期'] = set()
                        ner_dict['保质期'].add(text)
                        break
            ...
            for word in word_res:
                ...
                for prefix in prefix_list:
                    if prefix in word:
                        ...
                        if len(text) != 16:
                            continue
                        if '3C编号' not in ner_dict:
                            ner_dict['3C编号'] = set()
                        ner_dict['3C编号'].add(text)
                        break
            ...
            for word in word_res:
                ...
                for prefix in prefix_list:
                    if prefix in word:
                        ...
                        if len(text) == 0:
                            continue
                        if '化妆品备案编号' not in ner_dict:
                            ner_dict['化妆品备案编号'] = set()
                        ner_dict['化妆品备案编号'].add(text)
                        break
    ...
    def Predict(self, request, timeout):
        ...
        try:
            ...
            if ner_dict is not None and '品牌' in ner_dict:
                total_ner_dict['attrs']['品牌'].extend(ner_dict['品牌'])
